...
sys.path.append('/home/amarachi/CheXRelFormer/models/')
...
class DecoderTransformer_v3(nn.Module):
    ...
    def __init__(self, input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=True, 
        ...
        self.fc = nn.Linear(256,output_nc)
    ...
    def forward(self, inputs1, inputs2):
        ...
        x = self.global_avg_pool(_c)
        x = x.view(x.size(0), -1)
        #print("global pool shape: {}".format(x.shape))
        x = self.fc(x)
        x = self.softmax(x)
        return x

#Transormer Ecoder with x2, x4, x8, x16 scales
class EncoderTransformer_Last(nn.Module):
    def __init__(self, img_size=256, patch_size=3, in_chans=3, num_classes=3, embed_dims=[32, 64, 128, 256],
                 num_heads=[2, 2, 4, 8], mlp_ratios=[4, 4, 4, 4], qkv_bias=True, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[3, 3, 6, 18], sr_ratios=[8, 4, 2, 1]):
...
class Decoder_LastBLK(nn.Module):
    def __init__(self,  in_channels = [32, 64, 128, 256], output_nc=3, embed_dim=256):
        super(Decoder_LastBLK, self).__init__()
        self.in_channels =in_channels
        ...
        c1_in_channels, c2_in_channels, c3_in_channels, c4_in_channels = self.in_channels
        self.global_avg_pool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(c4_in_channels,output_nc)
        self.softmax = nn.Softmax(dim=1)


    def forward(self, x_1, x_2):
        x = self.global_avg_pool(self.diff_c4(torch.cat((x_1, x_2), dim=1)))
        x = x.view(x.size(0), -1)
        x = self.fc(x)
        x = self.softmax(x)
...
class DecoderTransformer_AbsDiff(nn.Module):
    """
    Transformer Decoder
    """
    def __init__(self, input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=True, 
                    in_channels = [32, 64, 128, 256], embedding_dim= 64, output_nc=3, 
                    decoder_softmax = False, feature_strides=[2, 4, 8, 16]):
        super(DecoderTransformer_AbsDiff, self).__init__()
        #assert
        assert len(feature_strides) == len(in_channels)
        assert min(feature_strides) == feature_strides[0]
    ...
    def forward(self, inputs1, inputs2):
        ...
        _c4_1 = self.linear_c4(c4_1).permute(0,2,1).reshape(n, -1, c4_1.shape[2], c4_1.shape[3])
        _c4_2 = self.linear_c4(c4_2).permute(0,2,1).reshape(n, -1, c4_2.shape[2], c4_2.shape[3])
        _c4   = torch.abs(_c4_1 - _c4_2)
        _c4_up= resize(_c4, size=c1_2.size()[2:], mode='bilinear', align_corners=False)

        # Stage 3: x1/16 scale
        _c3_1 = self.linear_c3(c3_1).permute(0,2,1).reshape(n, -1, c3_1.shape[2], c3_1.shape[3])
        _c3_2 = self.linear_c3(c3_2).permute(0,2,1).reshape(n, -1, c3_2.shape[2], c3_2.shape[3])
        _c3   = torch.abs(_c3_1 - _c3_2) + F.interpolate(_c4, scale_factor=2, mode="bilinear")
        _c3_up= resize(_c3, size=c1_2.size()[2:], mode='bilinear', align_corners=False)

        # Stage 2: x1/8 scale
        _c2_1 = self.linear_c2(c2_1).permute(0,2,1).reshape(n, -1, c2_1.shape[2], c2_1.shape[3])
        _c2_2 = self.linear_c2(c2_2).permute(0,2,1).reshape(n, -1, c2_2.shape[2], c2_2.shape[3])
        _c2   = torch.abs(_c2_1 - _c2_2) + F.interpolate(_c3, scale_factor=2, mode="bilinear")
        _c2_up= resize(_c2, size=c1_2.size()[2:], mode='bilinear', align_corners=False)

        # Stage 1: x1/4 scale
        _c1_1 = self.linear_c1(c1_1).permute(0,2,1).reshape(n, -1, c1_1.shape[2], c1_1.shape[3])
        _c1_2 = self.linear_c1(c1_2).permute(0,2,1).reshape(n, -1, c1_2.shape[2], c1_2.shape[3])
        _c1   = torch.abs(_c1_1 - _c1_2) + F.interpolate(_c2, scale_factor=2, mode="bilinear")
...
class CheXRelFormer_AbsDiffDecoderL(nn.Module):
    def __init__(self,  in_channels = [32, 64, 128, 256], output_nc=3, embed_dim=256):
        super(CheXRelFormer_AbsDiffDecoderL, self).__init__()
        self.in_channels =in_channels
        self.embedding_dim = embed_dim
        self.output_nc = output_nc
        c1_in_channels, c2_in_channels, c3_in_channels, c4_in_channels = self.in_channels
        self.global_avg_pool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(2*c4_in_channels,output_nc)
        self.softmax = nn.Softmax(dim=1)


    def forward(self, x_1, x_2):
        x = self.global_avg_pool(torch.abs(x_1 - x_2))
        x = x.view(x.size(0), -1)
        x = self.fc(x)
        x = self.softmax(x)
...
# CheXRelFormer:
class CheXRelFormer(nn.Module):

    def __init__(self, input_nc=3, output_nc=3, embed_dim=256, input_transform='multiple_select', decoder_softmax=False):
        super(CheXRelFormer, self).__init__()
        #Transformer Encoder
...
#CheXRelFormer_LastBLK:
class CheXRelFormer_LastBLK(nn.Module):

    def __init__(self, input_nc=3, output_nc=3, embed_dim=256, input_transform='multiple_select', decoder_softmax=False):
        super(CheXRelFormer_LastBLK, self).__init__()
        #Transformer Encoder
        self.embed_dims = [64, 128, 320, 512]
        self.depths     = [3, 3, 4, 3] 
        self.embedding_dim = embed_dim
        ...
        self.Tenc_x2 = EncoderTransformer_Last(img_size=256, patch_size = 7, in_chans=input_nc, num_classes=output_nc, embed_dims=self.embed_dims,
                 num_heads = [1, 2, 4, 8], mlp_ratios=[4, 4, 4, 4], qkv_bias=True, qk_scale=None, drop_rate=self.drop_rate,
                 attn_drop_rate = self.attn_drop, drop_path_rate=self.drop_path_rate, norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 depths=self.depths, sr_ratios=[8, 4, 2, 1])
        ...
        #Transformer Decoder
        self.decoder  = Decoder_LastBLK(in_channels = [32, 64, 128, 256],output_nc=3, embed_dim=embed_dim)
...
#CheXRelFormer_AbsDiff
class CheXRelFormer_AbsDiff(nn.Module):

    def __init__(self, input_nc=3, output_nc=3, embed_dim=256, input_transform='multiple_select', decoder_softmax=False):
        super(CheXRelFormer_AbsDiff, self).__init__()
        #Transformer Encoder
        self.embed_dims = [64, 128, 320, 512]
        self.depths     = [3, 3, 4, 3]
        self.embedding_dim = embed_dim
        ...
        #Decoder
        self.decoder = DecoderTransformer_AbsDiff(input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=False, 
                    in_channels = self.embed_dims, embedding_dim= self.embedding_dim, output_nc=output_nc, 
                    decoder_softmax = decoder_softmax, feature_strides=[2, 4, 8, 16])
    ...
    def forward(self, x1, x2):
        [fx1, fx2] = [self.Tenc_x2(x1), self.Tenc_x2(x2)]
        out = self.decoder(fx1, fx2)
        return out

# CheXRelFormer_AbsDiffL:
class CheXRelFormer_AbsDiffL(nn.Module):

    def __init__(self, input_nc=3, output_nc=3, embed_dim=256, input_transform='multiple_select', decoder_softmax=False):
        super(CheXRelFormer_AbsDiffL, self).__init__()
        #Transformer Encoder
        self.embed_dims = [64, 128, 320, 512]
        self.depths     = [3, 3, 4, 3]
        self.embedding_dim = embed_dim
        ...
        #Decoder
        self.decoder = CheXRelFormer_AbsDiffDecoderL(in_channels = [32, 64, 128, 256],output_nc=3, embed_dim=embed_dim)
...
# CheXRelFormer_Local:
class CheXRelFormer_Local(nn.Module):

    def __init__(self, input_nc=3, output_nc=3, embed_dim=256, input_transform='multiple_select', decoder_softmax=False):
        super(CheXRelFormer_Local, self).__init__()
        #Transformer Encoder
        self.embed_dims = [64, 128, 320, 512]
        self.depths     = [3, 3, 4, 3] 
        self.embedding_dim = embed_dim
